...
export class S3Service {
  ...
  AWS_BUCKET_NAME = this.configService.get<string>(EEnvVariables.AWS_BUCKET_NAME);
  AWS_BUCKET_REGION = this.configService.get<string>(EEnvVariables.AWS_BUCKET_REGION);

  s3Client = this.getClient();
  private getClient() {
    const region = this.configService.get<string>(EEnvVariables.AWS_BUCKET_REGION);
    const accessKeyId = this.configService.get<string>(EEnvVariables.AWS_ACCESS_KEY);
    const secretAccessKey = this.configService.get<string>(EEnvVariables.AWS_SECRET_ACCESS_KEY);
